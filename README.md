...
* **다중 로봇 협업 (Multi-AMR Collaboration)**:
  ...
  * **AMR 2 (2차 출동 및 조치)**: 복구 로봇으로서 AMR 1이 전송한 정밀 좌표를 받아 출동하여 도구 픽업 등 임무를 수행하고 복귀합니다.
* **실시간 웹 통합 관제 (Web Dashboard)**: Flask 기반의 웹 대시보드에서 4개의 카메라(웹캠 2, AMR 2) 뷰를 동시에 확인하고, SQLite DB에 저장된 감지 이력을 관리합니다. 영상 클릭 시 확대되는 모달 기능을 지원합니다.
...
* **실시간 AI 디텍션 시각화**: 화면상에 감지된 객체의 Bounding Box, Class Name, Confidence Score를 실시간으로 렌더링하여 보여줍니다.
* **이벤트 로그 모니터링 (DB 연동)**: SQLite DB(`detect_events.db`)와 연동하여 로봇별 최신 감지 이력을 상단에 요약 표시합니다. 하단의 **이벤트 목록(Event Log)**을 통해 최근 감지된 200개의 데이터(객체 이름, 발생 섹션, 발생 시각 등)를 스크롤하여 조회할 수 있습니다.
* **인터랙티브 모달 (Modal View)**: 특정 카메라 영상을 클릭하면 화면이 확대되는 모달 창을 지원하여, 관제자가 위협 요소를 정밀하게 확인할 수 있습니다.
...
* **OS**: Ubuntu 22.04
* **ROS**: ROS2 Humble
* **Hardware**: TurtleBot4 & iRobot Create 3 Base, RPLIDAR A1M8, OAK-D Camera, USB Webcams x 2
* **Python Packages**:
  ```bash
